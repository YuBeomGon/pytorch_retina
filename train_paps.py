...
def main(args=None):
    ...
    parser.add_argument('--saved_dir', help='saved dir', default='trained_models/resnet50_320_ex_bn/') 
    parser.add_argument('--gpu_num', help='default gpu', type=int, default=4) 
    ...
    pretrained_retinanet.load_state_dict(torch.load(PATH_TO_WEIGHTS,map_location=device), strict=False)
    ...
    for param, state in zip(pretrained_retinanet.classificationModel.parameters(), pretrained_retinanet.classificationModel.state_dict()) :
        #print(state)
        retinanet.classificationModel.state_dict()[state] = param      
    ...
    del pretrained_retinanet
        
    retinanet = torch.nn.DataParallel(retinanet, device_ids = [4,3,2], output_device=GPU_NUM).to(device)
...
#                                 reg_loss=regression_loss.item(), avg_loss=total_loss/(step+1))
    ...
    torch.save(retinanet.state_dict(), parser.saved_dir + 'model.pt')
    ...
    # write output
    json.dump(results, open(parser.saved_dir + '{}_bbox_results.json'.format(dataset_val.set_name), 'w'), indent=4)
    # write GT
    json.dump(GT_results, open(parser.saved_dir + '{}_GTbbox_results.json'.format(dataset_val.set_name), 'w'), indent=4)     
    ...
    coco_pred = coco_true.loadRes(parser.saved_dir + '{}_bbox_results.json'.format(dataset_val.set_name))
    coco_gt = coco_true.loadRes(parser.saved_dir + '{}_GTbbox_results.json'.format(dataset_val.set_name))
